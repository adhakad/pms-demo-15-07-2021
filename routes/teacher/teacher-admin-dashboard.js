      ...
      getClass.exec((err, datas)=>{
        ...
        classTeacher.exec((err,result)=>{
          ...
          allClass.exec((err, allClass)=>{  
            if(result==null){
              if(datas==null){
                res.render('./teacher/teacher-admin-dashboard', { title: 'TechBista Solutions', msg:'',records:data,datas:'',result:'',allClass:allClass });
              }else{
                res.render('./teacher/teacher-admin-dashboard', { title: 'TechBista Solutions', msg:'',records:data,datas:datas,result:'',allClass:allClass });
              }
            }else{
              if(datas==null){
                res.render('./teacher/teacher-admin-dashboard', { title: 'TechBista Solutions', msg:'',records:data,datas:'',result:result,allClass:allClass });
              }else{
                res.render('./teacher/teacher-admin-dashboard', { title: 'TechBista Solutions', msg:'',records:data,datas:datas,result:result,allClass:allClass });
              }
            } 
          });      
        });
    ...
    if(data){ 
      ...
      teacher.exec((err, datas)=>{
        ...
        if(start_date==1234567890){
          var checkClass=classModule.findByIdAndUpdate(class_obj_id,{start_date:Date.now()});
          checkClass.exec((err, clsData)=>{
            res.redirect('/room/'+req.session.room_id+'/'+teacher_id);
          });
        }else{
          res.redirect('/room/'+req.session.room_id+'/'+teacher_id);
        }
